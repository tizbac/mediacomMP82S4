...
#define SENSOR_NAME CONS(RK29_CAM_SENSOR_SP2518,_mp73)  
...
//HEQ
#define  SP2518_P0_0xdd  0x7c	//0x80 modify by sp_yjp,20130327
#define  SP2518_P0_0xde  0x9a	//0x95 modify by sp_yjp,20130327
...
{
	{0xfd , 0x00},
	{0x1b , 0x0a},	//0x02 modify by sp_yjp,20130116  0x02 0x0a 0x12 0x1a 
	{0x0e , 0x01},
	{0x0f , 0x2f},
	{0x10 , 0x2e},
	{0x11 , 0x00},
	...
	//SI50_SP2518 UXGA 24MEclk 2.5PLL 1DIV 50Hz fix 11fps	//modify by sp_yjp,20130418
	//ae setting
	{0xfd , 0x00},
	{0x03 , 0x03},
	{0x04 , 0x5a},
	...
	{0xfd , 0x00},
	#else
	//SI50_SP2518 UXGA 24MEclk 2.5PLL 1DIV 60Hz fix 11fps	//modify by sp_yjp,20130418
	//ae setting
	{0xfd , 0x00},
	{0x03 , 0x02},
	{0x04 , 0xca},
	...
	{0x57 , 0x08},///raw_dif_thr	//0x08 modify by sp_yjp,20130327	
	{0x58 , 0x0a},///a	//0x08 modify by sp_yjp,20130327	
	{0x56 , 0x0c},///a	//0x08 modify by sp_yjp,20130327	
	{0x59 , 0x10},
		///R\B通道间平滑
	...
	{0xac , 0xb9},///b8 	//0xc0	modify by sp_yjp,20130327
	{0xad , 0xc7},///d5 	//0xc0	modify by sp_yjp,20130327
	{0xae , 0x00},///f2
	{0xaf , 0xf2},///e8
	{0xb0 , 0xa6},///95
	{0xb1 , 0xe8},///03
	...
	#if 1	//modify by sp_yjp,20130327
	{0xd3 , SP2518_P0_0xd3},
	{0xd4 , SP2518_P0_0xd4},
	{0xd6 , SP2518_P0_0xd6},
	{0xd7 , SP2518_P0_0xd7},
	...
	{0xfd , 0x00},
	{0xdd , SP2518_P0_0xdd},
	{0xde , SP2518_P0_0xde},///a0	//0x95 modify by sp_yjp,20130327
	{0xdf , 0x80},
...
static  struct reginfo sensor_SceneNight[] =
{
	...
	#if SP2518_FLICK_50HZ
	//SI50_SP2518 UXGA 24MEclk 2.5PLL 1DIV 50Hz fix 6fps	//modify by sp_yjp,20130418
	//ae setting
	{0xfd , 0x00},
	{0x03 , 0x01},
	{0x04 , 0xd4},
	...
	#else
	//SI50_SP2518 UXGA 24MEclk 2.5PLL 1DIV 60Hz fix 6fps	//modify by sp_yjp,20130418
	//ae setting
	{0xfd , 0x00},
	{0x03 , 0x01},
	{0x04 , 0x86},
...
static int sensor_init(struct v4l2_subdev *sd, u32 val)
{
   ...
   /* ret = sensor_write(client, 0x12, 0x80);
    ...
    //todo:get mirror from rkcamera_platform_data
    if(sensor->sensor_gpio_res->gpio_flag & 0x10000){
    	sensor_write(client, 0xfd , 0x00);
		sensor_write(client, 0x31 , 0x70);
    }else{
		sensor_write(client, 0xfd , 0x00);
		sensor_write(client, 0x31 , 0x10);
	}
...
static int sensor_s_fmt(struct v4l2_subdev *sd, struct v4l2_mbus_framefmt *mf)
{
	...
	#if 0	//modify by sp_yjp,20130402
	...
	switch (cmd)
	{
		...
		case RK29_CAM_SUBDEV_IOREQUEST:
		{
            ...
            if (sensor->sensor_io_request != NULL) { 
                char * name = dev_name(icd->pdev);
                sensor->sensor_gpio_res = NULL;
                for (i=0; i<RK29_CAM_SUPPORT_NUMS;i++) {
                    if (sensor->sensor_io_request->gpio_res[i].dev_name && 
                        (strcmp(sensor->sensor_io_request->gpio_res[i].dev_name, name) == 0)) {
                        sensor->sensor_gpio_res = (struct rk29camera_gpio_res*)&sensor->sensor_io_request->gpio_res[i];
                        break;
                    }
                }
                if (sensor->sensor_gpio_res == NULL) {
                    struct rkcamera_platform_data * camera = sensor->sensor_io_request->register_dev_new;
                    while (strstr(camera->dev_name,"end")==NULL) {
                        if(!strcmp(camera->dev_name, name)) {
                            sensor->sensor_gpio_res = &camera->io;
                            break;
                        }
                        camera++;
                    }
