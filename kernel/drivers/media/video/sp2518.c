...
/*
	...
	///SP2518 UXGA 24MEclk 3��Ƶ 1��Ƶ 50Hz fix 10fps
	{0xfd , 0x00},
	{0x03 , 0x03},
	{0x04 , 0x66},
	{0x05 , 0x00},
		...
		case Sensor_PowerDown:
		{
			if (icl->powerdown) {
				...
				if (ret == RK29_CAM_IO_SUCCESS) {
					if (on == 0) {
						mdelay(100);    // 20 modify by sp_yjp,20120831
						if (icl->reset)
							icl->reset(icd->pdev);
						mdelay(10);    //50 add by sp_yjp,20120831
					}
...
static int sensor_init(struct v4l2_subdev *sd, u32 val)
{
	...
	msleep(10);	//30	 modify by sp_yjp,20130923
	...
	msleep(100);	//50  	modify by sp_yjp,20130923
   ...
   /* ret = sensor_write(client, 0x12, 0x80);
    ...
    msleep(200);	//add by sp_yjp,20130923
