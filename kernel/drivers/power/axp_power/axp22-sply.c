...
static int axp_debug = 0;//change by wangjian1;
...
bool axp_usbcurflag = 0;
bool axp_usbvolflag = 0;

// yftech
static bool usb_pc_connected;
void yftech_usb_pc_connect(bool connected)
{
	usb_pc_connected = connected;
	schedule_delayed_work(&usbwork, 0);
    return 0;
}
EXPORT_SYMBOL_GPL(yftech_usb_pc_connect);

// 充电模式时，设置关屏充电电流
void yftech_charger_mode(bool enable)
{
	axp_charger->charger_mode = true;
}
// end
...
static void axp_charger_update_state(struct axp_charger *charger)
{
  ...
  axp22_ac_valid = !!charger->ext_valid;
}
        ...
        void *data)
{
    ...
    if((bool)data==0){
		if(axp_debug){
			...
			if ((event & (AXP22_IRQ_ACIN|AXP22_IRQ_ACOV|AXP22_IRQ_ACRE)) && charger->ac.dev)
				power_supply_changed(&charger->ac);
			if ((event & (AXP22_IRQ_USBIN|AXP22_IRQ_USBRE)) && charger->usb.dev)
...
{
	uint8_t tmp;
	int cur = axp_charger->charger_mode ? SUSCHGCUR : EARCHGCUR;
	DBG_PSY_MSG("======early suspend=======cur=%duA\n", cur);
...
static void axp_charging_monitor(struct work_struct *work)
{
	...
	if(axp_debug){
		...
		axp_read(charger->master,0x10,temp_val);
		DBG_PSY_MSG("Axp22 REG_10 = %x\n",temp_val[0]);
		axp_read(charger->master,0x29,temp_val);
		DBG_PSY_MSG("Axp22 REG_29 = %x\n",temp_val[0]);
...
static void axp_usb(struct work_struct *work)
{
	...
	uint8_t tmp,val;
	struct axp_charger *charger;
	static unsigned long charger_start_time = 0;
	...
	// yftech
	axp_usbvolflag = axp_usbcurflag = true;
	if (charger->usb_valid) {
		if (charger_start_time == 0) {
			charger_start_time = jiffies + msecs_to_jiffies(9* 1000);
		} else {
			if (!usb_pc_connected && time_before(charger_start_time, jiffies)) {
				axp_usbvolflag = axp_usbcurflag = false;		// usb adaper
			}
		}
	} else {
...
// yftech
	...
	var = (val & 0x0f) * 150 + 300;
	printk("elf current=%d mA\n", var);

	axp_read(charger->master, 0x30, &val);
	var = val & 0x03;
	// 00 -900mA, 01-500mA, 1x- no current limit
	if (var >= 0x2) {
		printk("elf VBUS current limit=%s\n", "no current limit");
	} else {
		printk("elf VBUS current limit=%s\n", (var==0) ? "900mA" : "500mA");
	}
	if (val&0x40) {
		// 000: 4.0V 001: 4.1V
		printk("elf VBUS VHOLD voltage limit=%dmV\n", ((val>>3)&0x07) * 100 + 4000);
	} else {
		printk("elf VBUS VHOLD no voltage limit\n");
	}

	printk("elf axp_usbvolflag=%d, usb_pc_connected=%d, charger->usb_valid=%d\n", axp_usbvolflag, usb_pc_connected, charger->usb_valid);
#endif
// end
}
...
//probe 时初始化RDC，使其提前计算正确的OCV，然后在此处启动计量系统
  ...
  /* set usb cur-vol limit*/
	...
	schedule_delayed_work(&usbwork, msecs_to_jiffies(20 * 1000));	// yftech, 插USB开机需要时间
	/*给局部变量赋值*/
	axp_charger = charger;
#ifdef CONFIG_HAS_EARLYSUSPEND
