					...
					struct axp_platform_data *pdata)
{
	...
	axp_write(&axp->dev, 0x12, 0xff);//enable dc1sw
	axp_read(&axp->dev,0x12,&val);
	printk("#################set dc1sw on %d\n",val);
	axp_write(&axp->dev, 0x90, 0x03);//enable gpio0
	//axp_write(&axp->dev, 0x30, 0xfb);///set usb charge
...
// yftech
void yftech_axp_power_dc1sw(bool on)
{
	if (!axp) {
		printk("error! %s.%d\n", __func__, __LINE__);
		return;
	...
	if (on) {
		axp_set_bits(&axp->dev, 0x12, 0x80);
	} else {
		axp_clr_bits(&axp->dev, 0x12, 0x80);
	}
}
				  ...
				  const struct i2c_device_id *id)
{
	...
	if (gpio_request(AXP_IRQNO, "axp22") != 0) {
		printk("axp gpio_request error\n");
	}
	gpio_pull_updown(AXP_IRQNO, GPIOPullUp);

	client->irq = gpio_to_irq(AXP_IRQNO);
