...
#define IRQ_PORT			goodix_info.irq_pin
#define WAKE_PORT			goodix_info.rest_pin

#define SCREEN_MAX_X	env_get_u32("lcd_h_vd", 1024)
#define SCREEN_MAX_Y	env_get_u32("lcd_v_vd", 768)
...
//		gpio_free(WAKE_PORT);
		printk("WAKE_PORT gpio_request error\n");
//		return -EIO;
	}
...
//		gpio_free(IRQ_PORT);
		printk("IRQ_PORT gpio_request error\n");
//		return -EIO;
	}
			...
			const struct i2c_device_id *id)
{
	...
	gslX680_init();
	if (init_chip(client) < 0) {
		gpio_free(WAKE_PORT);
		gpio_free(IRQ_PORT);
		dev_err(&client->dev, "Not find!\n");
		return -ENODEV;
	}
...
static struct i2c_board_info i2c_info_dev =  {
	I2C_BOARD_INFO(GSLX680_I2C_NAME, GSLX680_I2C_ADDR),
	.platform_data	= NULL,
};

static void add_ctp_device(void) {
	int twi_id = 2;
	struct i2c_adapter *adap;

	adap = i2c_get_adapter(twi_id);
	i2c_info_dev.addr = GSLX680_I2C_ADDR;
...
static int __init gsl_ts_init(void)
{
	...
	if(!ctp_supported(GSLX680_I2C_NAME))
		return 0;
