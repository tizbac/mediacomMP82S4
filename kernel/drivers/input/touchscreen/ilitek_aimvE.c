	...
	if(i2c.protocol_ver & 0x200){
		...
		for(i=0; i<len; i++){
			...
			if(ilitek_i2c_read(i2c.client, ILITEK_TP_CMD_READ_SUB_DATA, buf, 5)){
				...
				if(revert_x_flag) x = i2c.max_x - 1 - x;
				if(revert_y_flag) y = i2c.max_y - 1 - y;
	...
	struct i2c_client *client, pm_message_t mesg)
{
	...
	if(i2c.valid_irq_request != 0){
		...
		ilitek_i2c_transfer(client, msgs, 1);
		msleep(5);
	...
	void)
{
	...
	if(exchange_x_y_flag) {
		swap(i2c.max_x, i2c.max_y);
		swap(i2c.x_ch, i2c.y_ch);
	}
	...
	void)
{
	...
	if(ret == 0){
		...
		revert_x_flag = env_get_u32("ctp_ili_revert_x", 0);
		revert_y_flag = env_get_u32("ctp_ili_revert_y", 0);
		exchange_x_y_flag = env_get_u32("ctp_ili_exchange_x_y", 0);
	...
	void)
{
	...
	use_interupt=env_get_u32("ctp_ilitek_use_interupt", 1);
