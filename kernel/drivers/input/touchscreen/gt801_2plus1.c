 ...
 *                    < 0; err
 */
static void ctp_fetch_sysconfig_para(void)
{
	screen_max_x = env_get_u32("lcd_h_vd", 1024);
	screen_max_y = env_get_u32("lcd_v_vd", 768);

	revert_x_flag = env_get_u32("gt801_revert_x_flag", 0);
	revert_y_flag = env_get_u32("gt801_revert_y_flag", 0);
	exchange_x_y_flag = env_get_u32("gt801_exchange_x_y_flag", 0);
	twi_id = env_get_u32("gt801_twi_id", 2);
	twi_addr = env_get_u32("gt801_twi_addr", 0x55);
...
static int goodix_ts_power(struct goodix_ts_data * ts, int on)
{
	...
	else if(on == 1)		//resume
	{
		...
		msleep(20);
		if(ts->use_irq) 
			gpio_direction_input(INT_PORT); //Set IO port as interrupt port 
		else 
			gpio_direction_input(INT_PORT);
		//msleep(260);
...
static int goodix_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
...
#else
	ts->input_dev->absbit[0] = BIT_MASK(ABS_MT_TRACKING_ID) | BIT_MASK(ABS_MT_TOUCH_MAJOR)|
	...
	err = request_irq(TS_INT, goodix_ts_irq_handler, IRQF_TRIGGER_RISING | IRQF_SHARED, client->name, ts);
	if (err < 0) {
