 ...
 * License version 2, as published by the Free Software Fou.ndation, and
...
#define	GPIO_IRQ (goodix_info.irq_pin)
...
err_free_allocate:
    ...
    if (ts->use_irq)
    {
	    ...
	    ret = check_gpio_interrupt((ts->client->irq);	//interrupt pin is still LOW, read data until interrupt pin is released.
	    if (!ret)
	    {
...
/* Report co-ordinates to the multi-touch stack */
	...
	for(i = 0; ((i < TPInfo->fingers) && (i < MAX_FINGERS)); i++)
	{
	         reportx = TPInfo->pt[i].x;// * 1024/4096;
	         reporty = TPInfo->pt[i].y;//* 768/4096;
	         //printk(KERN_INFO "chaoban test000: reportx = %d, reporty = %d \n",reportx,reporty);	
		if(TPInfo->pt[i].Pressure)
		{


		//input_mt_slot(ts->input_dev, TPInfo->pt[i].id);//wsp
			...
			input_report_abs(ts->input_dev, ABS_MT_PRESSURE, TPInfo->pt[i].Pressure);
			input_report_abs(ts->input_dev, ABS_MT_POSITION_X, reportx/*TPInfo->pt[i].x*/);
			input_report_abs(ts->input_dev, ABS_MT_POSITION_Y, reporty /*TPInfo->pt[i].y*/);
			input_report_abs(ts->input_dev, ABS_MT_TRACKING_ID, TPInfo->pt[i].id);     //Android 2.3
			input_mt_sync(ts->input_dev);
			all_touch_up = false;
		}
...
err_free_allocate:
	    //printk(KERN_INFO "err_free_allocate: 000ts->use_irq =%d \n",ts->use_irq);
    if (ts->use_irq)
    {
#ifdef _INT_MODE_1 //case 1 mode
...
{
	int ret;
	ret = gpio_request(GPIO_IRQ, "TS_INT");    //Request IO
	if (ret < 0)  {
		printk("Failed to request GPIO:%d, ERRNO:%d\n",GPIO_IRQ, ret);
	}
	else {
		gpio_direction_input(GPIO_IRQ);
		gpio_pull_updown(GPIO_IRQ, 1);
	}
	return ret;
}
...
int sis_sent_zero_command(struct i2c_client *client)
{
...
static int sis_ts_power(int on)
{
	if(on) {
	...
	struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	struct goodix_platform_data *pdata = &goodix_info;
	int err = 0;
    printk(KERN_INFO "sis_ts_probe\n");

	if(!ctp_supported(SIS_I2C_NAME)) {
		return -ENODEV;
	}
	...
	ret = sis_sent_zero_command(client);
	if(ret < 0) {
		return -ENODEV;
	}

	ctp_register(SIS_I2C_NAME);
   ...
   // pdata = client->dev.platform_data;
	...
	ts->input_dev->name = "sis_touch";//"sis_touch";
	...
	if (client->irq < 0) 
	{
		printk("<hk> %s:(%d) %s ======================================= irq is null! \n", __FILE__, __LINE__, __FUNCTION__);
	}
	else
	{
		ts->client->irq = gpio_to_irq(GPIO_IRQ);
		ret = request_irq(ts->client->irq, sis_ts_irq_handler, IRQF_TRIGGER_FALLING, client->name, ts);
		printk("<hk> %s:(%d) %s w======================================= ret:%d \n", __FILE__, __LINE__, __FUNCTION__, client->irq);
		if (ret == 0)  
		{
		   ts->use_irq = 1;
		}
...
static int __devinit sis_ts_init(void)
{
	...
	printk( KERN_INFO "sis_ts_init sis_wq ok\n" );
