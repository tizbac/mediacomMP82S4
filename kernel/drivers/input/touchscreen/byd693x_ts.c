...
struct byd_ts_priv {
	...
	u8 btn_pre_TPKey;
	int suspend_opend;
	int Backup_Finger_X[FINGER_NO_MAX];
	int Backup_Finger_Y[FINGER_NO_MAX];
};
	...
	if ((btn_status & 0x0f))
		{
			for(i=0;i< (btn_status & 0x0f);i++)
			{
				...
				if((0xa0 == Finger_Status) || (0x90 == Finger_Status))		//0xa0:The first Touch;  0x90: Hold Finger Touch
				{
					...
					byd_priv->Backup_Finger_X[Finger_ID] = xpos;
					byd_priv->Backup_Finger_Y[Finger_ID] = ypos;
...
//					printk("Finger_ID = 0x%x, DOWN\n", Finger_ID);
				...
				{
					Report_Status = 1;
					xpos = byd_priv->Backup_Finger_X[Finger_ID];
					ypos = byd_priv->Backup_Finger_Y[Finger_ID];
...
static void byd693x_load_param(void)
{
	...
	Get_Finger_Num = env_get_u32("ctp_byd6932_pn", 5);
